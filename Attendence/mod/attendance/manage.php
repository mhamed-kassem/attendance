...
require_once ('classes/db_Connect/db_Transactions.php');
$notifymasssage= new db_Transactions();
$studentwillSentWarning = null;

$Student_id= $notifymasssage->getall_stundents();

for($i=0;$i < count($Student_id) ;$i++)
{
    $checkAsbense = $notifymasssage->checkStudent_AsbenseTwo($Student_id[$i]);

    if($checkAsbense != null)
    {
        if($studentwillSentWarning != null)
        {
            $status=0;
            for($y=0 ; $y<count($studentwillSentWarning) ;$y++){
                if($studentwillSentWarning[$y][0] == $checkAsbense[0][0])
                {
                    $status = 1;
                }
            }
            if($status != 1)
            {
                $studentwillSentWarning[count($studentwillSentWarning)][0]=$checkAsbense[0][0];
                $studentwillSentWarning[count($studentwillSentWarning)][1]=$checkAsbense[0][1];
            }
        }else{
            $studentwillSentWarning[0][0]=$checkAsbense[0][0];
            $studentwillSentWarning[0][1]=$checkAsbense[0][1];
        }
    }
...
if($studentwillSentWarning != null)
{
    require_once ('classes/OserverPattern/Instructor.php');
    require_once ('classes/OserverPattern/NotifyApplied.php');


    for($x=0;$x<count($studentwillSentWarning);$x++)
    {
        $messageContent="Course name is:".$course->fullname." /   Student_id :".$studentwillSentWarning[$x][0].'/ message Content : We warning you for Your frequent absence ';

        $instructor=new Instructor($messageContent,$notifymasssage);
        $notify=new NotifyApplied($studentwillSentWarning[$x][1],$studentwillSentWarning[$x][0],0);
